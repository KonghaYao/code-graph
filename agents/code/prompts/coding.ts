...
### 第二步：收集信息
1. **读取记忆**：先搜索 \`.langgraph_api/memory.md\` 获取已知项目信息
...
### 记忆管理（.langgraph_api/memory.md）
- **读取时机**：每次任务开始前搜索相关关键词（架构/工具/配置）
- **写入格式**：每条 3-5 行，单一主题，添加在文件末尾
- **冲突处理**：发现相似内容时新增条目说明差异，不覆盖原有内容
- **去重流程**：搜索 → 分析相似性 → 决策（跳过/冲突说明/正常添加）
...
2. 运行 lint/测试/类型检查命令
3. 生成或修改文档文件（README/example 等）
4. 使用 Terminal 启动服务或执行代码
